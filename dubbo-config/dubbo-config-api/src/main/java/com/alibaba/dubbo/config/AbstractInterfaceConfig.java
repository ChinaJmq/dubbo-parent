...
public abstract class AbstractInterfaceConfig extends AbstractMethodConfig {
    ...
    protected void checkApplication() {
        ...
        // 初始化优雅停机的超时时长，参见 http://dubbo.apache.org/books/dubbo-user-book/demos/graceful-shutdown.html 文档。
    ...
    /**
     * 组装注册中心URL参数，将ApplicationConfig，RegistryConfig中属性组装到URL的属性里
     * @param provider true:服务者 false:消费者
     * @return
     */
            ...
            for (RegistryConfig config : registries) {
                ...
                if (address.length() > 0 && !RegistryConfig.NO_AVAILABLE.equalsIgnoreCase(address)) {
                    ...
                    for (URL url : urls) {
                        ...
                        //若是服务消费者，判断是否只注册不订阅。如果是，不添加到结果 registryList 。对应 《Dubbo 用户指南 —— 只注册》 文档
    ...
    protected void checkStubAndMock(Class<?> interfaceClass) {
        ...
        /**
         * 如果stub=true,则stub实现类的类名必须为接口名+Stub，如果stub直接引用具体类即Stub=com.foo.BarServiceStub，那么stub实现累得类名不限制
         */
        if (ConfigUtils.isNotEmpty(stub)) {
            ...
            try {
                //验证stub类中是否含有带有public (BarService barService) {this.barService = barService;}
                ReflectUtils.findConstructor(localClass, interfaceClass);
            } catch (NoSuchMethodException e) {
                throw new IllegalStateException("No such constructor \"public " + localClass.getSimpleName() + "(" + interfaceClass.getName() + ")\" in local implementation class " + localClass.getName());
            }
        ...
        if (ConfigUtils.isNotEmpty(mock)) {
            if (mock.startsWith(Constants.RETURN_PREFIX)) {//验证mock="return null"
            ...
            } else {//验证mock="true"或者mock="com.foo.BarServiceMock"
