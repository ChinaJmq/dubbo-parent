...
final class HeaderExchangeChannel implements ExchangeChannel {
    ...
    /**
     * 发送请求
     * 返回 DefaultFuture 对象。从代码的形式上来说，有点类似线程池提交任务，返回 Future 对象
     * @param request
     * @param timeout
     * @return
    ...
    public void close(int timeout) {
        ...
        //标记 closed = true ，避免发起多次新的请求,确保关闭请求只发送一次
        closed = true;
        if (timeout > 0) {
            long start = System.currentTimeMillis();
            /**
             * 1.已发起的请求已经都响应了,即 DefaultFuture.hasFuture(channel)返回fasle直接关闭
             * 2.已发起的请求未响应，但是时间超过了timeout直接关闭
             */
